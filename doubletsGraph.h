...
namespace main_savitch_15
{
    ...
    public:
        ...
        void add_vertex(const Item& label);
        void add_edge(const Item& source, const Item& target);
        // CONSTANT MEMBER FUNCTIONS
        std::size_t size( ) const { return many_vertices; }
        bool is_edge(const Item& source, const Item& target) const;
        std::set<Item> neighbors(const Item& vertex) const;
        int get_Index(const Item& str)
        {
          for(int i = 0; i < many_vertices; i++)
          {
            if(edges[i]->data() == str) return i;
          }
          return -1;
        }
        std::vector<Item> shortest_path(const Item& source, const Item& dest);
    private:
        std::vector<node<Item> *> edges;
        std::vector<Item> labels;
        std::size_t many_vertices;
    };
}

#include "doubletsGraph.template" // Include the implementation.
#endif
